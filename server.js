...
wss.on('connection', function connection(ws) {
  ...
  ws.on('message', function incoming(message) {
    ...
    // Quando a ESP32 enviar "stop", envia a resposta ao cliente externo
    if (message === 'stop') {
...
app.post('/enviar-comando', (req, res) => {
  const comando = req.body;  // Espera receber apenas '0' ou '1' diretamente no corpo

  // Verifica se o comando é válido (0 ou 1)
